...
class VehicleServiceTest {
	...
	void update_noPermiteEditarVehiculoDeOtroUsuario_devuelve404() {
		...
		when(vehicleRepository.findByIdVehicleAndCustomer_IdCustomer(1L, 999L)).thenReturn(Optional.empty());
		...
		assertThatThrownBy(() -> vehicleService.update(email, 1L, dto)).isInstanceOf(ResponseStatusException.class)
				.satisfies(ex -> {
