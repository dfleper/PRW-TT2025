...
public interface AppointmentRepository extends JpaRepository<AppointmentEntity, Long> {
	...
	@Query("""
			    ...
			    where a.estado <> :cancelada
			      ...
			      and a.fin > :inicio
			""")
	boolean existsOverlap(@Param("inicio") LocalDateTime inicio, @Param("fin") LocalDateTime fin,
	...
	@Query("""
			    ...
			    where a.employeeAsignado = :employee
			      ...
			      and a.fin > :inicio
			""")
	boolean existsOverlapForEmployee(@Param("employee") EmployeeEntity employee, @Param("inicio") LocalDateTime inicio,
			@Param("fin") LocalDateTime fin, @Param("cancelada") AppointmentStatus cancelada);

	// ========= Listado de citas del cliente (JOIN FETCH para evitar
	...
	// ========= Detalle cliente seguro con JOIN FETCH (evita
	// LazyInitializationException) =========
	@Query("""
			    select distinct a
			    from AppointmentEntity a
			...
			""")
	Optional<AppointmentEntity> findDetailByIdAndCustomer(@Param("id") Long id, @Param("customerId") Long customerId);
	...
	@Query("""
			    ...
			    order by a.inicio asc
			""")
	List<AppointmentEntity> findAgendaByDay(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
	...
	@Query("""
			    ...
			    order by a.inicio asc
			""")
	List<AppointmentEntity> findAgendaByDayAndStatus(@Param("start") LocalDateTime start,
			@Param("end") LocalDateTime end, @Param("estado") AppointmentStatus estado);

	// ========= HISTORIAL POR VEHÍCULO (CLIENTE) =========
	// Citas FINALIZADA del vehículo, ordenadas por inicio desc.
	// Carga service/vehicle y workOrder (si existe) para evitar
	...
	@Query("""
			    ...
			    order by a.inicio desc
			""")
	List<AppointmentEntity> findFinalizedHistoryByVehicleId(@Param("vehicleId") Long vehicleId,
			@Param("estado") AppointmentStatus estado);

	// ========= BACKOFFICE: agenda diaria (derivados - opcional) =========
	// Si algún día quieres no usar joins (no recomendado para Thymeleaf si accedes
	// a relaciones)
	List<AppointmentEntity> findByInicioGreaterThanEqualAndInicioLessThanOrderByInicioAsc(LocalDateTime start,
			LocalDateTime end);

	List<AppointmentEntity> findByInicioGreaterThanEqualAndInicioLessThanAndEstadoOrderByInicioAsc(LocalDateTime start,
			LocalDateTime end, AppointmentStatus estado);
	...
	@Query("""
			    ...
			    where a.id = :appointmentId
			      ...
			      and a.estado = :estado
			""")
	Optional<AppointmentEntity> findHistoryDetailFinalizedByIdAndCustomer(@Param("appointmentId") Long appointmentId,
			@Param("customerId") Long customerId, @Param("estado") AppointmentStatus estado);
