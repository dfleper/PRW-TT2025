...
public class ClienteAppointmentController {
	...
	public ClienteAppointmentController(VehicleRepository vehicleRepository, ServiceRepository serviceRepository,
			UserRepository userRepository, CustomerRepository customerRepository, AppointmentService appointmentService,
	...
	public String listado(Model model) {
		...
		List<AppointmentEntity> appointments = appointmentRepository.findListByCustomerWithJoinsDesc(customerId);

		Set<Long> cancelableIds = new HashSet<>();
		for (AppointmentEntity a : appointments) {
			boolean canCancel = a.getEstado() != AppointmentStatus.CANCELADA
					&& a.getEstado() != AppointmentStatus.FINALIZADA && a.getEstado() != AppointmentStatus.EN_CURSO;
			if (canCancel)
				cancelableIds.add(a.getId());
		}
	...
	public String detalleCita(@PathVariable Long id, Model model) {
		...
		boolean canCancel = appointment.getEstado() != AppointmentStatus.CANCELADA
	...
	private void reloadFormLists(Model model) {
		...
		model.addAttribute("vehicles", vehicleRepository
				.findByCustomer_IdCustomerAndActivoTrueOrderByMarcaAscModeloAscMatriculaAsc(customerId));

		model.addAttribute("services", serviceRepository.findByActivoTrueOrderByNombreAsc());
