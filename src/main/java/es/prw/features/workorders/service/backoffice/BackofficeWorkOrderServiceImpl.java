...
public class BackofficeWorkOrderServiceImpl implements BackofficeWorkOrderService {
	...
	public List<WorkOrderListRow> listWorkOrders(String status, String q) {
		...
		return workOrderRepository.findListForBackoffice(statusEnum, q).stream().map(wo -> {
			...
			return new WorkOrderListRow(wo.getId(), toUiStatus(wo.getEstado()), wo.getCreatedAt(), wo.getClosedAt(),
					customerName, cu.getEmail(), v.getMatricula(), s.getNombre(),
	...
	public List<WorkOrderListRow> listOpenAssignedTo(Long mechanicEmployeeId, String q) {
		...
		return workOrderRepository.findOpenAssignedToMechanic(mechanicEmployeeId, WorkOrderStatus.cerrada, q).stream()
				.map(wo -> {
					var a = wo.getAppointment();
					var v = a.getVehicle();
					var s = a.getService();
					...
					return new WorkOrderListRow(wo.getId(), toUiStatus(wo.getEstado()), wo.getCreatedAt(),
							wo.getClosedAt(), customerName, cu.getEmail(), v.getMatricula(), s.getNombre(),
							a.getEstado() == null ? "" : a.getEstado().name());
				}).toList();
	}
	...
	private WorkOrderStatus parseStatus(String status) {
		...
		if (s.isBlank() || s.equals("ALL"))
			return null;

		if (s.equals("OPEN"))
			return WorkOrderStatus.abierta;
		if (s.equals("CLOSED"))
			return WorkOrderStatus.cerrada;
